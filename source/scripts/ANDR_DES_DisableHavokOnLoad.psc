Scriptname ANDR_DES_DisableHavokOnLoad extends ReferenceAlias  
...
Event OnCellAttach()
	If (beenSimmed == False && getReference().Is3DLoaded()) 		
;		setMotionType(Motion_Keyframed, True) 		; edit by ANDR
		setMotionType(Motion_Keyframed, False) 		; edit by ANDR
; 		;debug.trace("havok disabled on: " + getReference)		
		getReference().MoveToMyEditorLocation()				; edit by ANDR	-> this has the object reset its position. Failsafe if the script loads too late.	
	EndIf	
EndEvent
...
Event OnLoad()
	If (beenSimmed == False && getReference().Is3DLoaded())		
;		setMotionType(Motion_Keyframed, True) 		; edit by ANDR
		(self.GetRef() as ObjectReference).setMotionType(Motion_Keyframed, False) 		; edit by ANDR
; 		;debug.trace("havok disabled on: " + getReference)
;		getReference().MoveToMyEditorLocation()				; edit by ANDR	-> this has the object reset its position. Failsafe if the script loads too late.	
	EndIf												
EndEvent
...
Function ReleaseToHavok()
		...
		getReference().ApplyHavokImpulse(0, 0, 1, 5)
