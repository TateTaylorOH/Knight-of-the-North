Scriptname DES_ANDR_DisableHavokOnLoad_RefAlias extends ReferenceAlias  
...
Modified by Tate Taylor to work on a ReferenceAlias}
...
;		setMotionType(Motion_Keyframed, True) 		; edit by ANDR
		Ref.setMotionType(GetReference().Motion_Keyframed, False) 		; edit by ANDR
; 		;debug.trace("havok disabled on: " + self)		
		Ref.MoveToMyEditorLocation()				; edit by ANDR	-> this has the object reset its position. Failsafe if the script loads too late.	
	EndIf	
EndEvent
...
ObjectReference Ref = GetReference()
	If (beenSimmed == False && Ref.Is3DLoaded())		
;		Ref.setMotionType(Ref.Motion_Keyframed, True) 		; edit by ANDR
		Ref.setMotionType(Ref.Motion_Keyframed, False) 		; edit by ANDR
; 		;debug.trace("havok disabled on: " + self)
;		Ref.MoveToMyEditorLocation()				; edit by ANDR	-> this has the object reset its position. Failsafe if the script loads too late.	
	EndIf												
EndEvent
